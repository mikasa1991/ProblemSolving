/*
n개로 이루어진 정수 집합에서 원하는 수 k 이상의 값 중에 가장 작은 자료의 인덱스를 출력하시오.
...
/*
k 이상의 값 중에 가장 작은 자료라는 것은, 결국 k값을 가지는 자료인데, 그렇다면 이진탐색이랑 다를 게 없잖아?

근데, k값이 2개 이상인 경우에는, 이진탐색처럼 풀 수가 없다.
이 때, 오름차순 정렬 기준으로, 가장 왼쪽에 있는 k값이 lower bound가 된다.
*/
